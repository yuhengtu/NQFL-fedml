...
def load_partition_data_mnist(batch_size,
    ...
    if partition_method == 'homo':
        logging.info('transforming data to homogeneity...')
        train_data = data_distribution_to_homo(train_data)
...
def data_distribution_to_homo(data):
    # data[u]['x'/'y']
    users = list(data.keys())
    all_x = data[users[0]]['x']
    all_y = data[users[0]]['y']
    for i in range(1, len(users)):
        all_x += data[users[i]]['x']
        all_y += data[users[i]]['y']
    ...
    data_num_each_client = int(len(all_x) / len(users))
    for i in range(len(users)):
        data[users[i]]['x'] = all_x[i * data_num_each_client:min((i + 1) * data_num_each_client, len(all_x))]
        data[users[i]]['y'] = all_y[i * data_num_each_client:min((i + 1) * data_num_each_client, len(all_x))]
    return data
